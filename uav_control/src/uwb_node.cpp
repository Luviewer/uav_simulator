...
void send_tcp(uint8_t* buff_send, uint16_t len) { send(sockfd, buff_send, len, 0); }

void send_tag(uint16_t* Dist, uint16_t* Rtls)
{
    ...
    send_buf[send_length++] = 0; //ÏÈ²»Ð´³¤¶È µÈ×îºóÔÙÐ´Èë

    send_buf[send_length++] = 0xAC;
    send_buf[send_length++] = 0xDA; //´ú±í±êÇ©Êý¾Ý°ü
...
int main(int argc, char** argv)
{
    ...
    ros::Rate loop_rate(10); // set the rate as defined in the macro MESSAGE_FREQ
    ...
    memset(Rtls, 0, sizeof(uint16_t) * 3);
